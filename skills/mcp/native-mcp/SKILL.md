...
Add MCP servers to `~/.openclaw/projects/MimirAether/config.yaml` under the `mcp_servers` key:
...
1. Reads `mcp_servers` from `~/.openclaw/projects/MimirAether/config.yaml`
...
No `mcp_servers` key in `~/.openclaw/projects/MimirAether/config.yaml`, or it's empty. Add at least one server.
