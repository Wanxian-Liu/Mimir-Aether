---
name: mimiraether-physics-reasoner
description: >
  Mimir 物理推理助手 — 路线 1（力学核心求解器）。
  自然语言 → 公式匹配 → SymPy 推导 → NumPy 求解 → 自然语言回报。
  基于杨立昆 JEPA（不预测像素，预测抽象表征）和李飞飞空间智能（物理接地）。
  CPU only，零新增依赖，不改 agent/gateway。
category: mimiraether
evolve_priority: high
evolve_confidence: 0.90
---
