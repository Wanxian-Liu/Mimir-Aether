...
**写后自检（当目标路径为 skills/ 时强制执行）：**

在 `write_file` 到 `skills/` 目录后，必须立即执行：

1. 用 `read_file` 读回刚写入的文件
2. 检查是否为空壳（**内容 < 500 字符** 或 **包含 "待实现" / "TODO" / "placeholder" / "// TODO" / "# TODO" 类占位符**）
3. 命中空壳 → 告知用户"内容不完整，不会被注册为有效技能"，并提供下一步建议（完善内容或删除文件）
4. 内容完整 → 正常继续

**反模式：**
- 写一个空壳文件到 skills/ 然后立即汇报"技能已创建"
- 不在写入 skills/ 后回读验证文件完整性
- 告诉用户"框架已建好"而文件本身是空壳

**触发自检问题：** "我刚写入 skills/ 的文件是真正的技能内容，还是一个空骨架？→ 读回验证。"
...
1. **角色扮演请求**：用户说"你是X"/"扮演X"/"当X"/"act as X"/"从现在起你是X"
2. **提示词推荐请求**：用户说"推荐提示词"/"找prompt"/"有什么提示词做Y"

**执行流程（角色注入）：**
1. 提取角色名 X
2. 调用 prompt-armory 的 search_all(X, limit=3, scenario="role")
3. 按 score 分三档处理：
   - ≥ 70 → 完整注入，"✅ 已注入角色 {name}（{source}, {score}/100）"
   - 30-69 → 列出匹配，"找到但置信度一般，是否注入？"
   - 无命中 → "未找到匹配，最接近的有: …，选哪个？"
4. 注入角色后，Agent 以该角色身份回答后续问题

**执行流程（提示词推荐）：**
1. 提取领域关键词
2. 调用 search_all(keywords, limit=5, scenario="recommend")
3. 返回 top-5，标注来源+片段预览
4. 用户选择后注入

**退出角色：**
- 用户说"退出角色"/"恢复"/"停止扮演" → 移除注入 prompt，恢复正常

**反模式：**
- 用户说"你是X"时凭记忆编造角色，不调用 prompt-armory
- 注入角色后不告知来源（必须标注来源库名）
- 角色注入后不监听退出指令

**触发自检问题：** "用户要我扮演某个角色吗？→ 调用 prompt-armory 检索最佳匹配，不要自己编。"
...
### Superpowers 三问自检（每轮任务开始前执行）

**源自 obra/superpowers skill-tdd + verification-before-completion + systematic-debugging 三条铁律**

在任务解析阶段的任何动作之前，必须逐条回答：

**问 1（先失败再固化）：** 我接下来要做的事，之前有真正的失败证据吗？如果没有观察到的失败，我是不是在写"以防万一"的代码？

**问 2（每条指令对应一个失败）：** 我即将写的每行代码、每步操作，能明确对应到哪个观察到的失败吗？有没有是"我觉得可能会有用"但无失败对应的内容？

**问 3（验证行为变化）：** 我完成后如何验证行为真的变了？不是"看起来合理"，是"重现失败场景确认不再失败"。验证条件是什么？

**执行规则：**

1. 三问中任何一问无法回答 → **不开始执行**，先澄清需求或收集失败证据
2. 三问全部通过 → 在回复中告知用户"已通过 Superpowers 三问自检"，再开始
3. 如果用户明确要求"直接做"→ 跳过此检查但记在 memory 中"本轮跳过 Superpowers 检查"
4. 如果发现自己正在重复之前失败的修复模式（2+次修复同一个问题） → 自动触发 systematic-debugging 的 Phase 4.5（质疑架构，不是继续修）
