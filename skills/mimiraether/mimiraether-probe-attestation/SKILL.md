...
### 坑 15 · 探针载体选错文件 ⇒ 伪负结论（\"job 从未执行\"）

**坏做法**：`grep <job_id> ~/.mimiraether/logs/gateway.log` ⇒ 最后命中 08-12 ⇒ 差点下结论「该 job 从不执行」。
**实际**：`gateway.log` 自 08-12 起**不再写 cron 行**（且已混入二进制），**现行 cron 行落在 `logs/agent.log`**
（同一天的 job 行在 agent.log 里有 30~2300 条）。**同一台机上同名日志文件不止一个在写，且旧的那个不会自报停写。**
...
```
**通用判据（与「探针通道 ≠ 对象通道」合流）**：
> 换一层就是**同一句话**：探针通道 ≠ 对象通道。
> 否定性结论前先问：**「我这个 grep 的文件，今天还在被写吗？」** 停写的文件给出的是「它自己停写了」，不是「对象不存在」。
