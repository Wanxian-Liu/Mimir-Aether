...
## ⚠️ 探针实施坑 · 第八批（2026-09-24 实测 · **分支顺序**决定归类 ⇒ 假 `control_mismatch`）

### 坑 16 · 分类器探针的分支若有重叠面，先跑到的那支就"赢了"（当场踩，1 次）

**场景**：wiki 质量门禁复核，写分类器 `classify(file, lineno) -> REAL_BROKEN | FP:*`
（判「报出来的 broken wikilink 是真断链还是已知误报面」）。首版分支顺序 =
`not_md → alias → 空目标/纯锚点 → inline_code → 查目标页`。

**症状**：`verdict=UNVERIFIED / reason=control_mismatch`，而**正控与负控都过**。
错的是 target：`concepts/pagerank-random-walk-lineage.md:216`（原文 = `` `[[ ]]` ``，
**反引号内的空目标示例**）被判 `FP:empty_or_anchor_only`，期望是 `FP:inline_code`。

**根因（双重）**：
1. **分支重叠**：该串**同时**满足「空目标」与「行内代码」两个条件，谁在前谁定名 —— 分类器不唯一。
2. **空串是上游造的**：脚本用 `while read link` 读链接，`read` 按 IFS **吃掉空白** ⇒
   `[[ ]]` 的链接体退化成空串，于是空目标分支反而比 `inline_code` 先命中。

**修法**：分支顺序改为**结构类优先、内容类在后** —— `not_md → inline_code → alias → 空目标/纯锚点 → 查目标页`。
重跑后 5 个 target 全中 ⇒ `VERIFIED`。

**通用判据（新增纪律）**：
> 分类器型探针的类别若**可能重叠**，必须先定**判定优先级**并写进探针说明；
> 只报 `FP:*` / `REAL_*` 这类**大类**的断言，与报**具体子类**的断言，鉴别力不同 ——
> **期望值写细到子类时，等于把"分类顺序"也写成了断言**，顺序一变就假红。

**元教训（与「先怀疑探针」并列的一条）**：本例 `UNVERIFIED` **不是**"世界没满足断言"，
而是"**我的探针命名法**与我的期望不一致"。
⇒ 探针被拒时先看 **reason**：`control_*_failed` = 量具有问题；`control_mismatch` 型
（正负控都过、只有 target 不匹配）= **期望/命名口径**有问题，**别去改结论**。

**顺带留痕纪律**：修好的探针**另 append 一条 VERIFIED**，并把被拒的那条（UNVERIFIED）**留在台账不删** ——
台账是 append-only，`476→479→…` 的**三类读数并存**本身就是闸在工作 的证据（同第七批旁证）。

## ⚠️ 探针实施坑 · 第九批（2026-09-24 实测 · **日志轮转**让固定单文件探针假 0）
...
**根因（真源读数）**：探针运行期间 `agent.log` **恰好轮转**：
`agent.log` 4.6 KB / 32 行（16:04:21 起）· `agent.log.1` 5.2 MB / 40550 行（09-16 10:31 → 09-24 16:04）。
所有历史命中都在 `.log.1` ⇒ 单文件探针读到的是**刚被清空的新文件**。
**判据**：`for f in agent.log*; do echo "$f $(head -1 $f|cut -c1-19) .. $(tail -1 $f|cut -c1-19)"; done`。

**修法**：扫描面覆盖**全轮转族**（`glob("logs/agent.log*")` 或 `grep -c … agent.log*`），
并在报告里**给每个载体打印时间范围**。修后正控=seen(3) ⇒ VERIFIED。
同族上游 = 坑 15（「最后一次写入」类读数不可用 `grep | tail` 定）——本坑是其**姊妹形态**：
坑 15 是「载体含二进制 ⇒ 行序失真」，本坑是「载体被换走 ⇒ 命中蒸发」。**共性：先定载体，再下结论。**

**通用自问句（第 6 条）**：**我这条探针读的文件，会不会在我读它的时候被搬走/截断？**
会 ⇒ 扫全族（`*` 通配）+ 打印各载体时间范围。

### 坑 18 · 负控夹具**自身**构成一次"读取"（我手写的夹具骗了我）

**场景**：同一次的 A 臂（AST 死变量探针：数「最后一次赋值之后的 `Name(Load)`」）。
负控夹具写 `messages.append({"role": "assistant", "content": content})` ——
`content` 作为**字典值**出现，本身就是一次 `Name Load` ⇒ 负控 observed=**seen** ⇒ `negative_control_failed`。

**纪律**：**负控失败的第一嫌疑人是夹具**（与「正控失败先怀疑探针形状」并列）。
造负控时自问：**我为了"让它看起来正常"写的那行代码，会不会恰好满足我在断言的条件？**
本坑与坑 13（探针扫到自己写的串）、「探针数到自己」同族 —— 载体换了三次：**进程表 → 自己的轨迹 → 自己手写的夹具**。

## ⚠️ 探针实施坑 · 第十批（2026-09-26 实测 · **调用层**连踩三次 ⇒ 探针根本没跑通）

> 场景：卡「陈旧标题已消失」这一条断言 **连跑 4 次才 VERIFIED，前 3 次全是命令串的构造问题**，
> 而三次症状**都长得像「负事实成立」**（正控空 / 负控恰好合格）。
> **共性（第 7 条通用自问句）：我这条探针命令，在到达被扫对象之前会经过几层 shell？每层会改写什么？**

### 坑 19 · 外层字符串格式化把 `{{ }}` **原样**送进 bash ⇒ `rc=127 command not found`

探针模板含 shell 花括号（如 `cmd | { grep -v '^0$' || true; }`）。若整段命令串由
**字符串格式化**拼出且写成 `{{ }}` 想转义 ⇒ **多数格式化实现不折叠 `{{`**，bash 收到 `{{`
⇒ 当成命令 ⇒ `rc=127` ⇒ **正控输出为空** ⇒ `positive_control_failed`。
**判据（决定性）**：读台账里 `controls.positive.cmd` **原文** + 同行 `rc`；`rc=127` 一眼定性 =
**命令不存在**，与「对象为空」无关。
**修法**：用**字符串拼接**，或写完先 `bash -n` 再手跑一次正控命令。

### 坑 20 · 探针模板里的 **shell 变量不会二次展开** ⇒ 恒 0（最安静的形态）

模板写成 `grep -c -F '{INPUT}' $CARD`，而 `$CARD` 只是**我脚本里的变量**；探针由**另一层 shell**
执行 ⇒ `$CARD` 未设 ⇒ 展开为空 ⇒ grep 改成**读空 stdin** ⇒ 输出 `0` ⇒ `none`。
**危险点**：负控（期望 none）**恰好合格** ⇒ 表面上「只有正控坏了」，极易误判成「样本不对」。
**修法**：**卡路径一律字面量写进模板**（生成模板时就完成插值），别指望运行期展开。
**自检**：打印台账里 `controls.positive.cmd`，把 `{INPUT}` 换上正控样本**手跑一遍**，应得 `1`。

### 坑 21 · 正控样本被**我自己本轮这次编辑**删掉（自伤型控制组过期）

我拿「某处仍未闭」当正控，而**上一分钟我刚把该处改成「已闭」** ⇒ 正控样本在盘上已不存在
⇒ `positive_control_failed`。**这不是探针坏，是我拿「修前世界」的样本去测「修后世界」。**
**纪律**：改完盘上文本再选自证正控时，正控样本必须从**当前盘面现摘**（`grep -m1`），
不得沿用改动前记忆里的串。凡「我本轮改过 X」⇒ X 是正控的**禁区**。

### 坑 22（正向技巧）· 探针一律**进程内直接调 `attest()`**，不走 CLI + 多层 shell

四次尝试里唯一一次一次通过的形态是**进程内调用**：

```python
from agent.probe_attest import attest       # cwd = repo, sys.path 加 repo
r = attest(claim="...",
           probe="bash -c \"grep -c -F '{INPUT}' " + CARD + " | { grep -v '^0$' || true; }\"",
           positive="...", negative="...", target="...", write=True)
print(r["verdict"], r["controls"]["positive"]["observed"], r["target"]["observed"])
```

**优点**：命令串由**语言层字符串**拼成 ⇒ 零引号转义、零变量展开、零格式化折叠；
正/负/目标全是 Python 字符串，**能写字面量就写字面量**。
**首选顺序**：进程内 `attest()` ＞ 落 `.sh` 文件（命令串只剩脚本路径）＞ 手搓 `bash -c` 嵌套引号。
后者 2026-09-26 三连败，**别再手搓**。

**旁证价值**：本次台账留 4 条读数（1 VERIFIED + 3 条不同 reason 的 UNVERIFIED），
**多类读数并存**本身就是闸在工作的证据（同第七/八批）—— 被拒条目**不要删**。
