...
If you already keep your X credentials in `~/.openclaw/projects/MimirAether/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.openclaw/projects/MimirAether/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `~/.openclaw/projects/MimirAether/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
