...
If you already keep your X credentials in `${MIMIR_AETHER_HOME:-$HOME/.mimiraether}/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf "${MIMIR_AETHER_HOME:-$HOME/.mimiraether}/.env" ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `${MIMIR_AETHER_HOME:-$HOME/.mimiraether}/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
