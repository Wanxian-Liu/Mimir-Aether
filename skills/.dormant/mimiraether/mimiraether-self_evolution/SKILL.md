...
## ⚠️ 仪器缺口（2026-09-19 实测 · 打任何分之前先读这一段）

本技能的 `collect_metrics()` 有两个**读出来恒为 0 的字段**，因为它们的量具**根本不存在**：

| 字段 | 读的文件 | 实测（2026-09-19） | 后果 |
|:--|:--|:--|:--|
| `verification_failures` | `data/verification_results.jsonl` | **文件不存在**（`test -e` → 0，RS17 探针 VERIFIED） | 静默返回 `{"total": 0, "by_type": {}}` |
| `repeat_tool_calls` | 同上 | 同上 | 静默返回 `0` |

⇒ **「无数据」被当成「无失败」**。这正是**探针失真族**：错误的表现形式是「看起来正常」
（metrics 全 0、analyze_gaps 找不到问题 ⇒ 报 healthy）。**它让自评永远看不到失败。**

**纪律（已固化为流程）**：
1. 跑 `collect_metrics()` 前先 `test -e` 这两个文件；任一缺失 ⇒ 该字段**标 N/A，不标 0**。
2. 凡「0 失败 / 0 事件」类结论，先跑 RS17 自证（正控 seen / 负控 none）——
   空文件、缺失文件、真无事件，三者在读数上不可区分。
3. 同类：`data/feedback_events.jsonl` 实测停更 **2026-08-16**（9 月零新增）
   ⇒ 反馈环整条链断，而断掉的表现是「没有反馈」，**在报表里不占任何一格**。

> 实证与本条来历：IQ 第三次复评（`~/src/MimirAether/docs/phase0/iq-scoring-rubric.md`「第三次复评」节
> + `notes/2026-09-19-IQ第三次复评与退化登记.md`）—— 总分 5.62 → **5.585 首次下跌**，
> 其中 **#3 反馈收集 −0.100**，全部源于这类「仪器死掉但静默」。
...
| **"metrics 全 0，说明系统健康"** | 0 也可能是**量具不存在**（见上表）。指标全 0 与「没测」在读数上不可区分 | 先 `test -e` 量具文件；缺失 ⇒ 标 N/A 并报「仪器缺口」，**不报 healthy** |
