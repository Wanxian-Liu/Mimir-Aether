...
## Critical: 格式符占位符字面量 bug 模式（A2 · 第 3 次复发 · 2026-09-18 定案）

**症状（两种表现，同一处根因）：**

1. **静默泄漏**：报告/日志/落盘文件里出现字面 `%s` / `%d`（未替换）。
2. **崩溃**：脚本抛 `ValueError: unsupported format character`（模板含字面 `%（` 又施加 `%` 运算）。

**为什么在「某一行」上修无效（结构化根因）：**
每支脚本自带一份 `log()` / `logline()` ⇒ **下一支新脚本又是一处新的漏传点**。
修的是那**一行**，不是那**一类**。⇒ 复发点必须修在**漏斗**上。

**机制化修复（唯一漏斗）：`~/src/MimirAether/scripts/report_template.py`（A2）**

- `render(tpl, *a)`：有参数 ⇒ 走 `%`，异常统一包成 `UnrenderedTemplateError`（带模板预览）；
  **无参数而模板含格式符（`%%` 除外）⇒ 当场报错**（这一闸就是消灭静默泄漏的那一闸）。
  字面百分号一律写 `%%`。
- `Report.write()`：落盘前对整篇跑 `scan_placeholders()` 终检，含泄漏**拒绝写入**（不产半成品文件）。
- `Report.raw()`：外部捕获文本（天然含 `%s`）走围栏显式豁免。
- **反例（勿重犯）**：不要再加「渲染后残留复检」兜底 —— 受控双探针实测它对
  「**参数值**里天然含 `%s`」的正样本**误报**；`%` 运算自身已保证所有格式符被消费。

**自证口径（三臂 · 同一字符串）**：① 旧 `log()`（应静默 + 落盘字面量=True）
② 新 `render()`（应抛领域异常）③ 补参 `render()`（应正常且不含字面量）。三读数可区分才算验过。

**同类延伸**：`***` 字面量（见上节）与本模式是**同一族**——「源码里出现了本应被替换的东西」。
判据都是**读盘原文/运行时断言**，不是凭输出截图定性。
