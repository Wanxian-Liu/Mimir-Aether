...
```bash
python "$(git rev-parse --show-toplevel)/optional-skills/mcp/fastmcp/scripts/scaffold_fastmcp.py" \
...
```bash
python "$(git rev-parse --show-toplevel)/optional-skills/mcp/fastmcp/scripts/scaffold_fastmcp.py" --list
```
...
- configure the server in `$MIMIR_AETHER_HOME/config.yaml` using the `native-mcp` skill, or
...
The server-building part may be correct while the Hermes config is not. Load the `native-mcp` skill and configure the server in `$MIMIR_AETHER_HOME/config.yaml`, then restart Hermes.
