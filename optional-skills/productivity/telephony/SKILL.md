...
- save provider credentials into `~/.openclaw/projects/MimirAether/.env`
...
### `~/.openclaw/projects/MimirAether/.env`
...
### `~/.openclaw/projects/MimirAether/telephony_state.json`
...
```bash
SCRIPT="$(find ~/.openclaw/projects/MimirAether/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
3. Buy it and save it into `~/.openclaw/projects/MimirAether/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.openclaw/projects/MimirAether/.env`
