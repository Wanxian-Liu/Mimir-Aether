...
- save provider credentials into `${MIMIR_AETHER_HOME:-$HOME/.mimiraether}/.env`
...
### `${MIMIR_AETHER_HOME:-$HOME/.mimiraether}/.env`
...
### `${MIMIR_AETHER_HOME:-$HOME/.mimiraether}/telephony_state.json`
...
SKILL_ROOT="${MIMIR_AETHER_HOME:-$HOME/.mimiraether}/skills"
if [ -n "$REPO" ] && [ -d "$REPO/optional-skills" ]; then
  SCRIPT="$(find "$REPO/optional-skills" "$REPO/skills" "$SKILL_ROOT" -path '*/telephony/scripts/telephony.py' -print -quit 2>/dev/null)"
else
  SCRIPT="$(find "$SKILL_ROOT" -path '*/telephony/scripts/telephony.py' -print -quit 2>/dev/null)"
fi
```
...
3. Buy it and save it into `${MIMIR_AETHER_HOME:-$HOME/.mimiraether}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `${MIMIR_AETHER_HOME:-$HOME/.mimiraether}/.env`
