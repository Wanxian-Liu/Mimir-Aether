...
- copy OpenClaw skills into `$MIMIR_AETHER_HOME/skills/openclaw-imports/` (runtime agent home; default `~/.mimiraether` when `MIMIR_AETHER_HOME` is unset)
- optionally copy the OpenClaw workspace instructions file into a chosen Hermes workspace
- mirror compatible workspace assets such as `workspace/tts/` into `$MIMIR_AETHER_HOME/tts/`
...
- `$MIMIR_AETHER_HOME/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `$MIMIR_AETHER_HOME/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `$MIMIR_AETHER_HOME/skills/migration/openclaw-migration/`.
2. When working from a MimirAether git clone, you can run the copy in-repo: `$(git rev-parse --show-toplevel)/optional-skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`.
3. If the installed path fails, inspect the installed skill directory and resolve the script relative to the installed `SKILL.md`.
4. Only use `find` as a fallback if the installed location is missing or the skill was moved manually.
5. When calling the terminal tool, do not pass `workdir: "~"`. Use an absolute directory such as the user's home directory, or omit `workdir` entirely.
...
```bash
python3 "$(git rev-parse --show-toplevel)/optional-skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py"
```
...
```bash
python3 "$(git rev-parse --show-toplevel)/optional-skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py" --preset user-data
```

Execute a user-data migration:

```bash
python3 "$(git rev-parse --show-toplevel)/optional-skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py" --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 "$(git rev-parse --show-toplevel)/optional-skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py" --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 "$(git rev-parse --show-toplevel)/optional-skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py" --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
- OpenClaw skills available under `$MIMIR_AETHER_HOME/skills/openclaw-imports/`
