...
for _attempt in $(seq 1 $((RETRY_COUNT > 0 ? RETRY_COUNT + 1 : 1))); do
  ...
  set +e
  (
    # --- A8/批4（2026-09-18 Mimir）: 全仓语法闸 + 归档域闸 ----------------------------
    # 批4（2026-09-18）：① `--root` 默认由 scripts/ 改为**本仓根**（补「空转绿盲区」：
    #   旧默认只扫 93 个文件，仓内 800+ .py 在视野外 —— 批4 首次真全仓扫即抓到
    #   agent/auto_retrospective.py 的孤儿三引号语法死）；
    #   ② 新增归档域 import 闸（清单单一真源 .mimir-archived-domains.txt，加行不改码）。
...
_gate2_pytest \
  ...
  tests/agent/test_agent_loop_reasoning_backfill.py \
  ...
  tests/agent/test_summary_llm_model_name.py \
  tests/agent/test_rs14_entity_carryover.py \
  tests/agent/test_probe_attest.py \
  tests/agent/test_compress_cooldown.py \
  tests/agent/test_compress_attribution.py
