#!/usr/bin/env python3
"""
MimirAether MCP Server — expose messaging conversations as MCP tools.

从Hermes mcp_serve.py适配。

Starts a stdio MCP server that lets any MCP client list conversations,
read message history, send messages, poll for live events, and manage
approval requests across all connected platforms.

Usage:
    mimiraether mcp serve
    mimiraether mcp serve --verbose
...
class EventBridge:
    """Background poller that watches SessionDB for new messages and maintains an event queue."""
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the MimirAether MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'mimiraether[mcp]'"
        )

    mcp = FastMCP(
        "mimiraether",
        instructions=(
            "MimirAether messaging bridge. Use these tools to interact with "
    ...
    def conversations_list(
        ...
        Returns conversations with their session keys, platform, chat type,
        display name, and last activity time.
    ...
    def messages_send(
        ...
        The target format is "platform:chat_id".
        ...
        try:
            ...
            return json.dumps({"error": "Send message tool not available in MimirAether"})
        except Exception as e:
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the MimirAether MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
