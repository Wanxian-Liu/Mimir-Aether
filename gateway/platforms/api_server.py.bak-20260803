...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                from mimiraether_cli.config import get_mimir_home
                db_path = str(get_mimir_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        try:
...
def _derive_chat_session_id(
    ...
    the same session sandbox directory across turns.
    """
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through MimirAether's AIAgent.
    """
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        3. Fallback: "MimirAether"
        """
        if explicit and explicit.strip():
            return explicit.strip()
        try:
            from mimiraether_cli.profiles import get_active_profile_name
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that sessions can be
        listed alongside CLI and gateway ones.
        """
        if self._session_db is None:
            try:
                from mimiraether_state import SessionDB
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the default.
        """
        from run_agent import AIAgent
        from gateway.run import _resolve_runtime_agent_kwargs, _resolve_gateway_model, _load_gateway_config
        from mimiraether_cli.tools_config import _get_platform_tools
        ...
        max_iterations = int(os.getenv("MIMIRAETHER_MAX_ITERATIONS", "90"))

        # Load fallback provider chain so the API server platform has the
        # same fallback behaviour as other platforms.
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if provided_session_id:
            ...
            # conversation map to the same session.  The first user
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from mimiraether_cli.auth import has_usable_secret
