...
第一步：判断消息类型——
- 若是【任务消息】（含"新任务""请写""写入""完成你的段""读X并输出到Y"等）：执行完整任务，**该写文件就写文件**（用write_file/patch落盘到指定路径），该验证就验证（grep/stat），任务要求的落盘产出必须完成。
- 若是【讨论消息】（含"立场""同意""回执""待命"等）：只需输出回复文本（50-200字，观点明确），不需要写文件。
- 若是【询问】消息：直接回答。
