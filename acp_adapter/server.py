"""ACP agent server — exposes MimirAether Agent via the Agent Client Protocol."""
...
class HermesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping MimirAether AIAgent."""
    ...
    _SLASH_COMMANDS = {
        ...
        "version": "Show MimirAether version",
    }
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show MimirAether version",
        },
    )
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate MimirAether using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="mimiraether-acp", version=MIMIRAETHER_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run AIAgent on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from mimir_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"MimirAether ACP Adapter v{MIMIRAETHER_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when MimirAether has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
...
# Alias: ACPServer = HermesACPAgent (for verification compatibility)
