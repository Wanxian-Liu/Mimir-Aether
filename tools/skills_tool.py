...
from mimiraether_constants import get_mimiraether_home as get_hermes_home
import os
import re
...
# All skills live in ~/.openclaw/mimir-aether/skills/ (seeded from bundled skills/ on install).
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return "Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to ~/.openclaw/mimir-aether/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.openclaw/mimir-aether/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.openclaw/mimir-aether/skills/ and external dirs.
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": "No skills found. Skills directory created at ~/.openclaw/mimir-aether/skills/",
                },
                ensure_ascii=False,
            )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.openclaw/mimir-aether/skills/): {skill_md}")
