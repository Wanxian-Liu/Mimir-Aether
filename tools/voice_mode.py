...
def _is_termux_environment() -> bool:
    """Check if running inside Termux (replaces hermes_constants.is_termux)."""
    ...
    def _is_container() -> bool:
        return (
            ...
            os.path.exists("/proc/1/cgroup") and (
                "docker" in open("/proc/1/cgroup").read() or
                "containerd" in open("/proc/1/cgroup").read()
            )
        )
    if _is_container():
...
_TEMP_DIR = os.path.join(tempfile.gettempdir(), "mimir_voice")
