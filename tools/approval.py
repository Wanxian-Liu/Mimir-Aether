...
def _load_yaml_config() -> dict:
    """Load config from config.yaml directly."""
    import yaml
    path = _config_path()
...
def _save_yaml_config(config: dict) -> None:
    """Save config to config.yaml directly."""
    import yaml
    path = _config_path()
...
def _smart_approve(command: str, description: str) -> str:
    ...
    NOTE: Smart approval is skipped in OpenClaw environment (no auxiliary_client).
    All dangerous commands escalate to manual approval.
    """
    logger.debug("Smart approvals: skipped (no auxiliary_client in OpenClaw), escalating")
    return "escalate"
