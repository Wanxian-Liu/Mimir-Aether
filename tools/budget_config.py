...
Overridable at the RL environment level via MimirAetherAgentEnvConfig fields.
...
class BudgetConfig:
    ...
    def resolve_threshold(self, tool_name: str) -> int | float:
        ...
        Raises ValueError if tool_overrides contains non-positive values.
        """
        if tool_name in PINNED_THRESHOLDS:
            return PINNED_THRESHOLDS[tool_name]
        if tool_name in self.tool_overrides:
            val = self.tool_overrides[tool_name]
            if not isinstance(val, int) or val <= 0:
                raise ValueError(
                    f"tool_overrides['{tool_name}'] must be a positive int, got {val!r}"
                )
            return val
