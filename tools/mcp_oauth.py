...
This module provides the glue:
    - ``MimirAetherTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "MimirAether"
"""
...
# ---------------------------------------------------------------------------
# MimirAetherTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class MimirAetherTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        MIMIRAETHER_HOME/mcp-tokens/<server_name>.json         -- tokens
        MIMIRAETHER_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to MimirAether.</p></body></html>"
            ) if code else (
                "<html><body><h2>Authorization Failed</h2>"
                f"<p>Error: {html.escape(error) if error else 'unknown'}</p></body></html>"
            )
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = MimirAetherTokenStorage(server_name)
    storage.remove()
...
def build_oauth_auth(
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "MimirAether")
    scope = cfg.get("scope")
