...
def _resolve_container_base(container_base: str | None) -> str:
    ...
    **historical** container layout (e.g. ``/root/.mimiraether`` or legacy
    ``/root/.openclaw/projects/MimirAether``) — migration/remap examples only,
    not the default runtime root; see ``docs/path-contract.md`` §历史路径与豁免目录.
    """
...
def _resolve_mimiraether_home() -> Path:
...
def register_credential_file(
    ...
    *relative_path* is relative to ``MIMIRAETHER_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside MIMIRAETHER_HOME so that a malicious
    ...
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to MIMIRAETHER_HOME)",
            relative_path,
        )
        return False

    host_path = mimiraether_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../.ssh/id_rsa`` cannot escape MIMIRAETHER_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, mimiraether_home)
...
def _load_config_files() -> List[Dict[str, str]]:
    ...
    try:
        import yaml
        mimiraether_home = _resolve_mimiraether_home()
        config_path = mimiraether_home / "config.yaml"
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if os.path.isabs(rel):
                        ...
                        continue
                    host_path = mimiraether_home / rel
                    containment_error = validate_within_dir(host_path, mimiraether_home)
                    if containment_error:
                        logger.warning(
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="mimiraether-skills-safe-"))
    ...
    def _cleanup():
        if _safe_skills_tempdir and _safe_skills_tempdir.is_dir():
            shutil.rmtree(_safe_skills_tempdir, ignore_errors=True)
            logger.info("credential_files: cleaned up symlink-safe skills temp dir")
...
def get_cache_directory_mounts(
    ...
    def _get_mimiraether_dir(new_subpath: str, old_name: str) -> Path:
        """Resolve a MimirAether subdirectory with backward compatibility.

        New installs get the consolidated layout (e.g. ``cache/images``).
        Existing installs that already have the old path (e.g. ``image_cache``)
        keep using it — no migration required.
        """
        old_path = mimiraether_home / old_name
        if old_path.exists():
            return old_path
        return mimiraether_home / new_subpath
