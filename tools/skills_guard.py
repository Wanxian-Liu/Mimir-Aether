...
Trust levels:
  - builtin:   Ships with MimirAether. Never scanned, always trusted.
...
THREAT_PATTERNS = [
    ...
    (r'\$(?:\{)?MIMIR_AETHER_HOME(?:\})?/\.env',
     "mimiraether_home_env_path", "critical", "exfiltration",
     "references .env via MIMIR_AETHER_HOME path expansion"),
...
def scan_file(file_path: Path, rel_path: str = "") -> List[Finding]:
    ...
    try:
        ...
        logger.warning("scan_file: failed to read %s: %s", file_path, e)
        return []
...
def should_allow_install(result: ScanResult, force: bool = False) -> Tuple[bool, str]:
    ...
    if force:
        return True, (
            f"⚠️ FORCE OVERRIDE: Installed despite {result.verdict} verdict "
