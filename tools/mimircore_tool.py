...
def _handle_produce_capsule(input_text: str, capsule_type: str = "auto", auto_publish: bool = True) -> str:
    """
    调用Mimir-Core生成胶囊
    ...
    try:
        ...
        if should_publish and auto_publish and capsule:
            try:
                ...
                publish_status = f"published: {filename}"
...
def _handle_list_capsules(tag_filter: str = None, limit: int = 20) -> str:
    ...
    try:
        ...
        items = [{"name": c.stem, "modified": time.strftime("%Y-%m-%d", time.localtime(c.stat().st_mtime))} for c in capsules]
...
def _handle_improve_capsule(capsule_id: str, improvement_hint: str) -> str:
    ...
    try:
        ...
        return _handle_produce_capsule(improved_text, capsule_type="optimize", auto_publish=True)
