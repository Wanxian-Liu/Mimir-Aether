...
Architecture:
    ~/.mimir/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        MIMIR_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = Path(os.environ.get("MIMIR_HOME", "~/.mimir")) / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("MIMIR_CHECKPOINT_TIMEOUT", "30"))))
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "checkpoint@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "MimirAether Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "MIMIR_WORKDIR").write_text(
