#!/usr/bin/env node
/**
 * Buzz 监听器 Mimir v2 —— 接收 + 发送 双通道（自动回执）
 *
 * v2 新增：
 *   1. sendToChannel() —— 发消息回频道（补齐"只收不发"缺口）
 *   2. 收到@我的消息 → 自动回复【REPLY】确认 + 写收件箱
 *   3. 收到频道@ → 提示收件箱有新任务（触发agent处理）
 *
 * 用法: BUZZ_SK=<hex> [BUZZ_CHANNEL=<channel-id>] node buzz-mimir-v2.js [agent名]
 */
...
console.log(`🐝 [${AGENT}] Buzz监听v2启动 pub=${pub.slice(0, 12)}... (接收+发送)`);
...
// 发送消息到频道
function sendToChannel(content, mentionPub = null) {
  const tags = [['h', CHANNEL], ['client', 'buzz-mimir-v2']];
  if (mentionPub) tags.push(['p', mentionPub]);
  ...
  console.log(`📤 [${AGENT}] 已发频道: "${content.slice(0, 50)}"`);
}
...
function handleEvent(ev) {
  ...
  if (content.includes('【REPLY】')) return;  // 防循环
  ...
  // 自动回执：收到@我的消息 → 回复频道
  if (pTag) {
    const reply = `【REPLY】收到${from}的消息，已写入收件箱。`;
    sendToChannel(reply, ev.pubkey);
  }
}
