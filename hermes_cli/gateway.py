"""
Gateway subcommand for mimir CLI.

Handles: mimir gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from gateway.hermes_status import terminate_pid
...
# when hermes_constants is cached from a pre-update version during `mimir update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "mimir-gateway*",
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``mimir update``
...
def stop_profile_gateway() -> bool:
    ...
    """
    try:
        from gateway.hermes_status import get_running_pid, remove_pid_file
    except ImportError:
        return False
...
_SERVICE_BASE = "mimir-gateway"
SERVICE_DESCRIPTION = "MimirAether Gateway - Messaging Platform Integration"
...
def _profile_arg(hermes_home: str | None = None) -> str:
    ...
    For ``~/.mimir/profiles/<name>``, returns ``"--profile <name>"``.
...
def get_service_name() -> str:
    ...
    Default ``~/.mimir`` returns ``mimir-gateway`` (backward compatible).
    Profile ``~/.mimir/profiles/coder`` returns ``mimir-gateway-coder``.
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    mimir gateway uninstall")
    print_info("    sudo mimir gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            ...
            if run_as_user:
                print_info(f"  After setup, run: sudo mimir gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo mimir gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo mimir gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.mimir`` → ``ai.mimir.gateway.plist`` (backward compatible).
    Profile ``~/.mimir/profiles/coder`` → ``ai.mimir.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.mimir.gateway-{suffix}" if suffix else "ai.mimir.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.mimir/mimir-agent  -> /home/alice/.mimir/mimir-agent
      /opt/mimir                 -> /opt/mimir  (kept as-is)
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    root's home.  This translates it to the target user's equivalent path:
      /root/.mimir                    → /home/alice/.mimir
      /root/.mimir/profiles/coder     → /home/alice/.mimir/profiles/coder
      /opt/custom-mimir               → /opt/custom-mimir  (kept as-is)
    """
    current_hermes = get_hermes_home().resolve()
    current_default = (Path.home() / ".mimir").resolve()
    target_default = Path(target_home_dir) / ".mimir"

    # Default ~/.mimir → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.mimir → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.mimir) — keep as-is
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}mimir gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}mimir gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    refresh_systemd_unit_if_needed(system=system)
    from gateway.hermes_status import get_running_pid
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}mimir gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}mimir gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}mimir gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.mimir.gateway-{suffix}" if suffix else "ai.mimir.gateway"
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  mimir gateway status             # Check status")
...
def launchd_stop():
    ...
    # `mimir gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @mimir:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. mimir) and copy the bot token",
    ...
    },
    {
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Hermes will use (e.g., mimir@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "6. To authorize users, use DM pairing: mimir pairing generate bluebubbles",
...
def _runtime_health_lines() -> list[str]:
    ...
    try:
        from gateway.hermes_status import read_runtime_status
    except Exception:
        return []
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'mimir pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: mimir pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'mimir gateway setup'")
            continue
...
def _setup_weixin():
    ...
    print_info("  3. Hermes will store the returned account_id/token in ~/.mimir/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `mimir gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `mimir pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `mimir pairing approve`.")
    elif access_idx == 1:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: mimir gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  mimir gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: mimir gateway run")
                print_info("  For persistence:   tmux new -s mimir 'mimir gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: mimir gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup mimir gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: mimir gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'mimir gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: mimir gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: mimir gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s mimir 'mimir gateway run'")
                print()
            ...
            print()
            print("  mimir gateway run                              # direct foreground")
            print("  tmux new -s mimir 'mimir gateway run'         # persistent via tmux")
            print("  nohup mimir gateway run > ~/.mimir/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: mimir gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: mimir gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: mimir gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: mimir gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: mimir gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  mimir gateway install")
                    print("  sudo mimir gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  mimir gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup mimir gateway run > ~/.mimir/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s mimir 'mimir gateway run'         # persistent via tmux")
                    print("  nohup mimir gateway run > ~/.mimir/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  mimir gateway install  # Install as user service")
                    print("  sudo mimir gateway install --system  # Install as boot-time system service")
