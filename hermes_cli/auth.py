"""
Multi-provider authentication system for MimirAether.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.mimir/auth.json with cross-process file locking.
...
DEFAULT_NOUS_CLIENT_ID = "mimir-cli"
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `mimir model` to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.mimir/auth.json
# =============================================================================
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `mimir logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'mimir doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'mimir model' for available providers, or run 'mimir doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'mimir model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.mimir/.env.",
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.mimir/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hermes auth store (~/.mimir/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `mimir auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `mimir auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `mimir auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `mimir auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Hermes auth store (~/.mimir/auth.json)."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `mimir auth` to re-authenticate."
            )
            relogin_required = True
        raise AuthError(
            message,
    ...
    *,
    ...
    try:
        ...
        if cli_tokens:
            ...
            print("   Run `mimir auth` to create a fully independent session.\n")
    ...
    *,
    ...
    if effective_ca:
        ...
        if not os.path.isfile(ca_path):
            import logging
            logging.getLogger("mimir.auth").warning(
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `mimir status` output.

    Checks the credential pool first (where the dashboard device-code flow
    and ``mimir auth`` store credentials), then falls back to the legacy
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `mimir auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `mimir auth` and
    # `mimir model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def login_command(args) -> None:
    """Deprecated: use 'mimir model' or 'mimir setup' instead."""
    print("The 'mimir login' command has been removed.")
    print("Use 'mimir auth' to manage credentials,")
    print("'mimir model' to select a provider, or 'mimir setup' for full setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.mimir/auth.json."""
    ...
    *,
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `mimir model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            ...
            print("Run `mimir model` or configure an API key to use Hermes.")
    else:
