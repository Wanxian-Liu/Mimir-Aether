...
Per-delta regex stripping leaks or over-strips when tags span chunks. This
module holds partial tags until the next delta or ``flush()`` resolves them.

Tag variants (case-insensitive except REASONING_SCRATCHPAD name):
  ``<think>``, ``<thinking>``, ``<reasoning>``, ``<thought>``,
  ``<REASONING_SCRATCHPAD>``, and legacy ``<think>``.

Behaviour aligned with Hermes ``think_scrubber.py`` (see docs/hermes-comparison-detailed.md §2.17).
"""
...
__all__ = ["StreamingThinkScrubber", "strip_think_blocks"]


class StreamingThinkScrubber:
    """Incremental feed(delta) → visible text; ``flush()`` at EOF."""
...
def strip_think_blocks(content: str) -> str:
    """One-shot scrub for final assistant content (matches streaming semantics)."""
    scrubber = StreamingThinkScrubber()
    return scrubber.feed(content or "") + scrubber.flush()
