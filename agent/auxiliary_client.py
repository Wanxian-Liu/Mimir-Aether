...
_AUTH_JSON_PATH = _get_mimiraether_home() / "auth.json"
...
def _read_codex_access_token() -> Optional[str]:
    ...
    return None  # Codex hermes_cli auth removed — MimirAether independent
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from agent.provider_registry import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if nous.get("source") == "pool":
        ...
        model = _NOUS_MODEL
    # Free-tier check removed — MimirAether independent
    return (
        OpenAI(
...
def resolve_provider_client(
    ...
    try:
        ...
        logger.debug("provider_registry not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        # Copilot Responses API wrapping removed — MimirAether independent
