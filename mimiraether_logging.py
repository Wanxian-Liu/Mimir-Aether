"""Centralized logging setup for MimirAether.
...
from mimiraether_constants import get_config_path, get_hermes_home
...
def clear_session_context() -> None:
    ...
    Call when a conversation ends so subsequent log records don't carry
    a stale session tag.
    """
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_mimiraether_session_injector", False):
    ...
    _session_record_factory._mimiraether_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for log inspection.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_logging(
    *,
    ...
    """Configure the MimirAether logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the MimirAether home directory.  Falls back to
    ...
    # Import RedactingFormatter from agent.redact
    try:
        from agent.redact import RedactingFormatter
    except ImportError:
        # Fallback if redact.py is not available
...
def setup_verbose_logging() -> None:
    ...
    Called by the agent when ``verbose_logging=True``.
    """
    try:
        from agent.redact import RedactingFormatter
    except ImportError:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_mimiraether_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._mimiraether_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the mimiraether group. However, both _open() (initial creation)
    and doRollover() create files via open(), which uses the process umask —
    ...
    def __init__(self, *args, **kwargs):
        self._managed = self._detect_managed()
        super().__init__(*args, **kwargs)

    def _detect_managed(self) -> bool:
        """Detect if running in managed/NixOS mode."""
