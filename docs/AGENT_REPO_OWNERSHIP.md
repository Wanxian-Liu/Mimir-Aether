...
| 项 | 状态 |
|:--|:----|
| 历史旧提交署名非 Mimir（当日 24/25） — → **见 §8 归因边界** | 不追溯、不改写（回执 §三：断层是可解释的断层，比改写历史干净；以 2026-09-12 回执为分界点） |
...
## 8. 归因边界（A4 · 2026-09-13 刘哥裁定「按你推荐的建议来」）

> **⚠️ 归因边界**：**2026-09-13 之前**的提交，其 git 署名**不作为归因依据**——历史成因是「统一代提交」，**不是**作者归属声明。**2026-09-13 起**以 `Agent: <id>` trailer 为准（`mimir` / `hermes` / `openclaw` / `loki`）。

| 仓 | 分界点 | 之前的署名实况（实测） | 之后看什么 |
|:--|:--|:--|:--|
| `~/src/MimirAether`（公开仓 `Wanxian-Liu/Mimir-Aether`） | 2026-09-13 | 近 50 条 = 琬弦 48 / Mimir 2 | `Agent:` trailer（A2 同日启用） |
| `~/wiki`（四方共享 checkout） | 2026-09-13 | 近 30 条 = `MimirAether` 29（四人**共用** committer 身份） | `Agent:` trailer（待四方立规后启用）+ 卡片 `author:` |

**为何取整日「2026-09-13」而非精确时刻**：无歧义，且比精确时刻**更保守**——把 09-12 当天的提交也归入「不追溯」。

**为何对外不写「48/50 实为 Mimir 交付」**：公开仓读者拿不到内部卡与 run 记录，写这个数字只会引起「仓库署名不实」的怀疑，而不增加可核对性；真数字留在内部卡与本文件本节。

### 取值口径

| 口径 | 内容 |
|:--|:--|
| **可见** | `git log --format='%h %(trailers:key=Agent)'`（本仓）· 卡片 `author:`（wiki） |
| **不采信** | git `user.name`（共享仓恒为同一身份）· 正文自述 · 口头声明 |
