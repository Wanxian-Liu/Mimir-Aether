...
| H15 | Registry | enable/disable/search/get 语义 | **工具可见性**由 `get_tool_definitions(enabled_toolsets=…)` 等过滤（如 TUI：`tui_gateway/server.py` 使用 `enabled_toolsets`）；无独立 `agent/tool_registry.py` 那种 SQLite 表 | **OK** | `agent/test_tool_registry_api.py`；工具名集合 **`scripts/diff_tool_names_hermes_mimir.py`** | **DIFF**：Hermes = **toolset/definitions 管道**；Mimir = **双轨**（`tools/registry` + `agent/tool_registry` DB）。**名级 diff**：仅 Hermes 侧剩 **`feishu_*`（5）**；Mimir 侧 **`mimir_only`（25，as-of 2026-05-01）** — `search_web` 已 **去注册**，与 Hermes 对齐为 **`web_search`**；遗留调用经 **`register_tool_remap` + `core_loop` `route_tool_call`** 转发。对齐策略表 **[`docs/h15_mimir_only_decisions.md`](h15_mimir_only_decisions.md)**；当前快照 **`docs/parity_snapshots/h15_tool_names_diff_20260501.json`**（历史：`h15_tool_names_diff_20260506.json`）。`browser_*` 曾因 **`tools/browser_camofox.py` 缩进错误**未加载 — 已修。 |
| H16 | Delegate | 缺 task / 非 PENDING / 未知 CLI；真子进程成功/超时 | 明确失败或 False；超时 → FAILED | **OK** | `agent/test_delegate_subagent_semantics.py`；`agent/test_delegate_subagent_integration.py`（POSIX） | — |
