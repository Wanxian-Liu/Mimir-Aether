...
| **IQ-M5** | 末粒含代码改动 → tier0 PASS ✅ | ✅ | handoff 已合 `a0dc323`，tier0 677 PASS + 4 预先失败（与基线一致） |
...
| **I4 意图感知** | 4.0 | **4.5** | +0.5 | WM B3 REPLAN_CTX env 已开；IQ-31/32 handoff **已合** `a0dc323`（需 `MIMIR_WM_PREDICTOR=1` 生效） |
| **I5 元认知** | 4.0 | **4.5** | +0.5 | self-audit + brain_metrics 稳定产出；禁止等继续已固化 |
| **I6 工程可靠** | 7.0 | **7.0** | — | tier0 677 PASS（10/10 pre-existing failures），无新退化 |

**总分**：4.9 → **5.2** ✅（达到务实目标）

> **诚实声明**：+0.3 分中，+0.2 来自机制修复（进化管道的真实可用），+0.1 来自硬规则/技能引用习惯化。**handoff 已合 `a0dc323`，但 `MIMIR_WM_PREDICTOR=1` 运行数据需观察 7d 才能确认闭环。** 距刘哥战略 5.5 还差 **0.3**。
...
§11 IQ #17 提升链
   ...
   波次 4（P1 工程）：IQ-30 ~ IQ-34 全部 [x] ✅（handoff 已合入 `a0dc323`）
...
| **B5 LLM 预测器** | **不做**（2026-05-19） | [`wm-b5-llm-predictor-deferred.md`](./wm-b5-llm-predictor-deferred.md) — 非原先目标；保持规则 WM |
...
| 意图预测可用 | handoff（已合 `a0dc323`） | 生产可用 + `MIMIR_WM_PREDICTOR=1` 刘哥已开 | IQ-31/32/33/34 代码已在 main |
| 评估 ok% | 0%/7d | ≥1% | 观察窗太短 |
