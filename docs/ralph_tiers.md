...
| **Gate2** | Hermes-parity unit/integration harness (loops, CLI edges, tools) + **M3 垂直切片**（CLI + API） | `run_ralph_tier0.sh` 列表（含 `test_m3_cli_quick_task_slice`, **`test_m3_api_chat_slice`**, `test_code_execution_tool_schema`, `test_tool_registry_api` 等） |
