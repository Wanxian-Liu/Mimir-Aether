...
| `tools/code_execution_tool.py` | G2: `agent/test_code_execution_tool_env.py`（本地子进程 env / `PYTHONPATH`）；G2: `agent/test_code_execution_remote_mock.py`（远程 mock：**无 python3** 早退；**最小成功** 无 RPC；**exit 124/130**；**畸形 `req_*` JSON**；**file RPC**（沙箱工具单测 + **同脚本 `web_search`+`read_file`** + **`web_search`+`read_file`+`web_extract`（三工具）**；除 **`write_file`** native 覆盖）与 **`max_tool_calls`**）；G2: `agent/test_code_execution_tool_schema.py::test_execute_code_schema_mentions_only_enabled_tools`, `test_execute_code_schema_contains_import_examples` | 远程完整 RPC/ ship 文件路径可另增 mock 或 ext；远程不复制本地 `PYTHONPATH` 拼接语义已文档化 |
