...
**相关（本仓库）**：[path-contract.md](./path-contract.md)（路径三层）、[ralph_parity_contract_v1.md](./ralph_parity_contract_v1.md)（行为契约）、[ralph_roadmap_milestones.md](./ralph_roadmap_milestones.md)（M0–M6）、[成长路线图.md](../成长路线图.md)（阶段目标）、[RALPH_MODE.md](./RALPH_MODE.md)（Ralph 模式：三轮零失败迭代）、[MAINLINE_STATUS.md](./MAINLINE_STATUS.md)（主线进度快照，问进度时更新）。
...
| 工作树 | 典型路径 | 在本方案中的角色 |
|--------|----------|------------------|
| **主开发树（本仓库）** | `~/.openclaw/projects/MimirAether`（默认 agent home，见 `docs/path-contract.md`） | **完整运行时**：Agent 主循环、gateway、工具、Ralph 门禁、`docs/ralph_*` 契约与 `./run_ralph_tier0.sh`。 |
