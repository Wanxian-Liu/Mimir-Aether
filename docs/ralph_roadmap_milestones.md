...
**最小切片（本仓库已落地）**：`agent/auxiliary_client.py` 分类函数 + **`fixtures/m4_http/error_shapes.json`**（静态 HTTP 失败形状）+ **`scripts/refresh_m4_http_fixtures.sh`**；验收见 `docs/m4_auxiliary_http_slice.md` 与 `agent/test_m4_auxiliary_http_slice.py`（无外网、无 key）。**M4 工程表标绿**以本切片为准；VCR / 常驻 mock 服务为可选增强。
