...
from mimir_cli.auth import get_nous_auth_status
from mimir_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: mimir setup tools, mimir setup terminal, or mimir status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `mimir setup` and `mimir model`."""
    if not managed_nous_tools_enabled():
        return set()
