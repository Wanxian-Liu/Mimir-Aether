...
Add, remove, or reorder entries here — both `mimir setup` and
`mimir` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `mimir model`
    flow without maintaining a separate static list.
    """
    from mimir_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from mimir_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  mimir model, /model, /provider, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `mimir model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `mimir model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``mimir model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``mimir auth add openai-codex`` without
    ``mimir model``).
    """
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    try:
        from mimir_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``mimir model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from mimir_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``mimir_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from mimir_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from mimir_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from mimir_cli.copilot_auth import copilot_request_headers
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from mimir_constants import AI_GATEWAY_BASE_URL
