"""
Status command for mimir CLI.

Shows the status of all MimirAether components.
"""
...
from mimir_cli.auth import AuthError, resolve_provider
from mimir_cli.colors import Colors, color
from mimir_cli.config import get_env_path, get_env_value, get_hermes_home, load_config
from mimir_cli.models import provider_label
from mimir_cli.nous_subscription import get_nous_subscription_features
from mimir_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all MimirAether components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ MimirAether Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from mimir_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: mimir model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: mimir model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: mimir model)"
    ...
    if not terminal_env:
        ...
        # (mimir status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    if _is_termux():
        try:
            from mimir_cli.gateway import find_gateway_pids
        ...
        if gateway_pids:
            ...
            print("  Start with:   mimir gateway")
        ...
        if is_container():
            ...
            try:
                from mimir_cli.gateway import get_service_name
                _gw_svc = get_service_name()
            except Exception:
                _gw_svc = "mimir-gateway"
            try:
        ...
        from mimir_cli.gateway import get_launchd_label
        try:
            result = subprocess.run(
    ...
    print(color("  Run 'mimir doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'mimir setup' to configure", Colors.DIM))
    print()
