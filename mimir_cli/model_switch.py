...
- ``mimir_cli.providers``        -- canonical provider identity + overlays
- ``mimir_cli.model_normalize``  -- per-provider name formatting
...
from mimir_cli.providers import (
...
from mimir_cli.model_normalize import (
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if not new_model:
            if pdef.base_url:
                from mimir_cli.runtime_provider import _auto_detect_local_model
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from mimir_cli/models.py (OPENROUTER_MODELS,
    ...
    from mimir_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    from mimir_cli.providers import HERMES_OVERLAYS
    from mimir_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if not has_creds:
            try:
                from mimir_cli.auth import _load_auth_store
    ...
    # with `hermes model`).
    try:
        from mimir_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
