"""
Unified tool configuration for MimirAether.

`mimir tools` and `mimir setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.mimir/config.yaml under
...
from mimir_cli.cli_output import (  # noqa: E402 — late import block
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("tool_search",     "🔧 Tool Search",               "search tools and skills by intent"),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from mimir_cli.plugins import discover_plugins, get_plugin_toolsets
...
from mimir_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hermes_home()}/mimir-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in mimir-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "mimir-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "mimir-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `mimir tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by mimir tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (mimir-cli, mimir-telegram, etc.)
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.mimir/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `mimir tools` and `mimir setup tools`.
    ...
    print(color("  Guide: https://mimir-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print(color("  Changes take effect on next 'mimir' or gateway restart.", Colors.DIM))
    print()
