"""
Interactive setup wizard for MimirAether.
...
_DOCS_BASE = "https://mimir-agent.nousresearch.com/docs"
...
# display_hermes_home imported lazily at call sites (stale-module safety during mimir update)
...
from mimir_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print_info("  mimir config set model.provider custom")
    print_info("  mimir config set model.base_url http://localhost:8080/v1")
    print_info("  mimir config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'mimir setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    from mimir_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from mimir_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'mimir setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'mimir setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'mimir setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'mimir setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'mimir setup tools' to configure them,"
        )
        from mimir_constants import display_hermes_home as _dhh
    ...
    print()
    print(f"   {color('mimir setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('mimir setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('mimir setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('mimir setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('mimir setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('mimir config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('mimir config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('mimir config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('mimir', Colors.GREEN)}              Start chatting")
    print(f"   {color('mimir gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('mimir doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `mimir tools` and `mimir setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``mimir model``)
    ...
    provider added to ``mimir model`` is automatically available here.
    ...
    # Delegate to the shared mimir model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from mimir_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: mimir model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from mimir_cli.auth_commands import auth_add_command
    ...
    if _vision_needs_setup:
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'mimir setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.mimir/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'mimir setup tts')."""
    _setup_tts_provider(config)
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `mimir setup agent` later to customize.")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://mimir-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'mimir whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'mimir whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from mimir_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from mimir_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from mimir_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from mimir_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from mimir_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from mimir_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from mimir_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from mimir_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://mimir-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://mimir-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  mimir config edit")
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'mimir setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     mimir config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: mimir gateway install")
            else:
                print_info("  You can install later: mimir gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo mimir gateway install --system")
                print_info("  Or run in foreground:  mimir gateway")
        else:
            from mimir_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   mimir gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   mimir gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `mimir setup tools` and `mimir tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from mimir_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from mimir_cli.auth import get_active_provider
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: mimir claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: mimir claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hermes (use mimir claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      mimir setup           — full or quick (auto-detected)
      mimir setup model     — just model/provider
      mimir setup tts       — just text-to-speech
      mimir setup terminal  — just terminal backend
      mimir setup gateway   — just messaging platforms
      mimir setup tools     — just tool configuration
      mimir setup agent     — just agent settings
    """
    ...
    print(
        color(
            "│             ⚕ MimirAether Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your MimirAether installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if choice == 0:
            ...
            print_info("Exiting. Run 'mimir setup' again when ready.")
            return
        elif 2 <= choice <= 6:
    ...
    print()
    print_info("You can edit these files directly or use 'mimir config edit'")
...
def _resolve_hermes_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``mimir chat`` in a fresh process."""
    ...
    try:
        if importlib.util.find_spec("mimir_cli") is not None:
            return [sys.executable, "-m", "mimir_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch mimir chat now?", True):
        return

    chat_argv = _resolve_hermes_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch Hermes automatically. Run 'mimir chat' manually.")
        return
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'mimir setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    mimir setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  mimir setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'mimir setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'mimir setup gateway'.")
