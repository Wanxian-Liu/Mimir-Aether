...
TIPS = [
    ...
    f"SOUL.md at {_TIP_HOME}/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    ...
    f"Checkpoints use shadow repos in {_TIP_HOME}/checkpoints/ — your project's .git is never touched.",
    ...
    f"BOOT.md at {_TIP_HOME}/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    ...
    f"SOUL.md is auto-seeded with a default personality on first run. Edit {_TIP_HOME}/SOUL.md to customize.",
    ...
    f"The skills audit log at {_TIP_HOME}/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    f"Every interrupt during an agent run is logged to {_TIP_HOME}/interrupt_debug.log with timestamps.",
    ...
    f"MCP ${{ENV_VAR}} placeholders in config are resolved at server spawn — including vars from {_TIP_HOME}/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    f"The skills quarantine at {_TIP_HOME}/skills/.hub/quarantine/ holds skills pending security review.",
]
