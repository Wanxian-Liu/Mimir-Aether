...
Usage:
    python -m mimir_cli.main web          # Start on http://127.0.0.1:9119
    python -m mimir_cli.main web --port 8080
"""
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from mimir_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in ("anthropic", "claude-code"):
        ...
        try:
            from mimir_cli.auth import clear_provider_auth
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    from mimir_cli import auth as hauth
    if provider_id == "nous":
        from mimir_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    from mimir_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
...
async def get_logs(
    ...
    from mimir_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from mimiraether_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from mimir_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from mimir_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from mimir_cli.tools_config import (
