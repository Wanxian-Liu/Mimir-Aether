"""
Doctor command for mimir CLI.

Diagnoses issues with MimirAether setup.
"""
...
from mimir_cli.config import get_project_root, get_hermes_home, get_env_path
from mimir_constants import display_hermes_home

PROJECT_ROOT = get_project_root()
HERMES_HOME = get_hermes_home()
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.mimir or ~/.mimir/profiles/coder)

# Load environment variables from ~/.mimir/.env so API key checks work
...
from mimir_constants import OPENROUTER_MODELS_URL
...
from mimir_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.mimir/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `mimir`.
    ...
    # Check ~/.mimir/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'mimir setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'mimir setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'mimir setup' to create one")
                issues.append("Run 'mimir setup' to create .env")
    
    # Check ~/.mimir/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        try:
            from mimir_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'mimir setup' to migrate config")
                else:
                    issues.append("Run 'mimir doctor --fix' or 'mimir setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'mimir doctor --fix'")
        except Exception:
            pass

        # Validate config structure (catches malformed custom_providers, etc.)
        try:
            from mimir_cli.config import validate_config_structure
    ...
    try:
        from mimir_cli.auth import get_nous_auth_status, get_codex_auth_status
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'mimir doctor --fix' to checkpoint")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'mimir setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: mimir skills list)")

    from mimir_cli.config import get_env_value
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: mimir memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: mimir memory setup")
                issues.append("No Honcho API key — run 'mimir memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run mimir memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: mimir memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: mimir memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from mimir_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "mimir -p" in content:
                            _m = _re.search(r"mimir -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'mimir doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
